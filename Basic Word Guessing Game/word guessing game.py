...
parser = argparse.ArgumentParser(description = 'I will try to guess your thinking Word. Lets start.')
...
def integer(question):
	try:
		...
		print('WARNING:- Please enter a number between 2 and 22!')
		return False
...
def ask_for_length(confirm = True):
	...
	while word_length is False or word_length > maximum or word_length < minimum:
		...
		if word_length is not False:
			print('WARNING:- Your word should have a length between %d and %d' % (minimum, maximum))

		# Store users input
		word_length = integer('Enter your word length now:')
...
# Start game with a nice introduction
print('''
I will be attempting to guess a word you are thinking of!
All you have to do is think of a word with a length between %d and %d
